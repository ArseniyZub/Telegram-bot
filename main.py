...
bot = Bot("6181902724:AAE1ykDKN7KQ7JwMPgFQQnXpoXvOI2JAI5Y")
...
async def quit_handler(message: types.Message):
    ...
    links = soup.find_all("div", class_="mv-search-result-heading")
    if len(links) > 0:
    ...
    await bot.send_photo(message.chat.id, photo=photo, caption=f'Ссылка на статью: <a href="{url}">сюда</a>', parse_mode="HTML")
